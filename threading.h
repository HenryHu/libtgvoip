...
template <typename T>
class MethodPointer : public MethodPointerBase {
   public:
    ...
    virtual void Invoke(void* arg) { (obj->*method)(arg); }
...
#if defined(_POSIX_THREADS) || defined(_POSIX_VERSION) || defined(__unix__) || \
...
class Semaphore {
   public:
    ...
    void Acquire(int count) {
        for (int i = 0; i < count; i++) Acquire();
    }

    void Release(int count) {
        for (int i = 0; i < count; i++) Release();
    }
...
class Semaphore {
   public:
...
#else
        h = CreateSemaphoreEx(NULL, initValue, maxCount, NULL, 0,
    ...
    void Release() { ReleaseSemaphore(h, 1, NULL); }

    void Acquire(int count) {
        for (int i = 0; i < count; i++) Acquire();
    }

    void Release(int count) { ReleaseSemaphore(h, count, NULL); }
